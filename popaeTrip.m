...
NpatCT(1) = sum(Yfraclass==1);
NpatCT(2) = sum(Yfraclass==2);
NpatCT(3) = sum(Yfraclass==3);
NpatCT(4) = sum(Yfraclass==4);
...
Nrepdim = 3;        % 4
Nhidnodes = 128;     % 64
numIterations = 250;  % 1500
miniBatchSize = 512;    % 128
wtinit = 1;

split_training = 2;  %1 = split total   2 = split each class
Nsplit = 1;

    aa = clock;
    ID = round(100*aa(6));
    nam0 = strcat('LV',num2str(ID));

nam = strcat(nam0,'-',num2str(Nsplit),'-',num2str(numIterations),'-',num2str(miniBatchSize));
...
modelN = 1;   % 1 = TripletNN    2 = SimplexNN
if modelN == 1
    tic
    [dlnet1,dlnet2,LV] = TsiaMae(XTrain,YTrain,Nrepdim,Nhidnodes,validFraction,numIterations,miniBatchSize,wtinit);  % Training function call
    toc
else
    tic
    dlnet1 = TsiaMaeSimplex(XTrain,YTrain,Nrepdim,Nhidnodes,validFraction,numIterations,miniBatchSize,wtinit);  % Training function call
    toc
    % Note: 5-24-24  no dlnet2 of LV ... this is for simplex testing only
    
    dlXTrain = dlarray(single(XTrain),'SSCB');
    FXTrain = extractdata(forward(dlnet1,dlXTrain));
    
    Loc = Polytope(Nclass-1,Nclass);
    
    BaryT = BaryCentric(Loc',FXTrain);
    [~,I] = max(BaryT);
    BaryTAcc = sum(I==(cat2num(YTrain))')/numel(I);
...
for cloop = 1:Nclass0
    idx1 = round(mean(F1idx(fraclass==cloop)));
    LU1(idx1) = cloop;
    idx2 = round(mean(F2idx(Yfraclass==cloop)));
    LU2(idx2) = cloop;
end

cmF1 = confusemat(fraclass,LU1(F1idx));
cmF2 = confusemat(Yfraclass,LU2(F2idx));

for cloop = 1:Nclass0
    cmF1(:,cloop) = cmF1(:,cloop)/NpatC(cloop);
    cmF2(:,cloop) = cmF2(:,cloop)/NpatCT(cloop);
end
...
Stat.acc1 = acc1;
Stat.acc2 = acc2;
Stat.validFraction = validFraction;
Stat.Nrepdim = Nrepdim;
Stat.Nhidnodes = Nhidnodes;
