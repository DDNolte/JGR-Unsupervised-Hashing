% popaeC.m
% Converted from ppopae.m Oct. 30, 2022
% Create continuous transitions among 4 classes
%
% Calls:
%   siaMae.m
%   Calls:
...
[Ntrain,~] = size(Fracg);
r = logical(randbin(Ntrain,0.5));

XTraintmp(1,:,1,:) = Fracg(r,1:FVncol)';
YTraintmp = categorical(fraclass(r));

XTestmp(1,:,1,:) = Fracg(~r,1:FVncol)';
YTestmp = categorical(fraclass(~r));

Nseg = 10;
[Xinterp1,SimplexClass,Yclass1] = PermuteClassF(XTraintmp,YTraintmp,Nseg);
[Xinterp2,~,Yclass2] = PermuteClassF(XTestmp,YTestmp,Nseg);
% 
% [~,~,~,Ntot] = size(Xinterp0);
% r = logical(randbin(Ntot,0.5));
% 
% Xinterp1 = Xinterp0(1,:,1,r);
% SimplexClass = SimplexClass0(r);
% Yclass = Yclass0(r);
% 
% Xinterp2 = Xinterp0(1,:,1,~r);
% Yclass2 = Yclass0(~r);
% 
XTrain = Xinterp1;
YTrain = categorical(Yclass1);
% 
XTest = Xinterp2;
YTest = categorical(Yclass2);
% 
...
if split_training == 1
   [YTrain,fraclassplit,Nclassp] = spliRtrain(Yclass1,Nsplit); 
   [YTestmp,~,~] = spliRtrain(Yclass2,Nsplit); 
   YTest = YTestmp';
elseif split_training == 2
   [YTrain,fraclassplit,Nclassp] = splitrain(Yclass1,Nsplit); 
   [YTestmp,~,~] = splitrain(Yclass2,Nsplit); 
   YTest = YTestmp';
end
...
% Reconstruct subclass probabilities

Nlink = Nclass*(Nclass-1)/2;
Ndel = Nsplit/Nlink;

for iloop = 1:Nsplit
   partvec(iloop,:) = zeros(1,Nclass); 
end

ind = 0;
for cloop1 = 1:Nclass
    for cloop2 = cloop1+1:Nclass
        for sloop = 1:Ndel
            ind = ind + 1;
                partvec(ind,cloop1) = 1 - (sloop-1)/(Ndel-1);
                partvec(ind,cloop2) = (sloop-1)/(Ndel-1);
        end
    end
end

PartProb = cmnrm2'*partvec;
